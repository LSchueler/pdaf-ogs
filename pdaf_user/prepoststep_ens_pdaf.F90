...
! *** local variables ***
  ...
  CHARACTER(len=4) :: anastr          ! String for call type (initial, forecast, analysis)
...
! *******************
  ...
  !   DO member = 1, dim_ens
  !      field_local = ens_p(:, member)

  !      WRITE (ensstr, '(i4.4)') member

  !      OPEN(11, file = 'output/ens_'//TRIM(ensstr)//'_step'//TRIM(stepstr)//'_'//TRIM(anastr)//'.txt', status = 'replace')
  ...
  !   OPEN(11, file = 'output/state_step'//TRIM(stepstr)//'_'//TRIM(anastr)//'.txt', status = 'replace')
